...
async function getAnalysis(id) {
  ...
  const res = await fetch(`${apiUrl}/api/analysis/${id}`, {
    cache: "no-store",
  });
  if (!res.ok) {
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 md:px-6 py-12">
        <div className="w-full max-w-4xl">
          <Alert variant="destructive">
            ...
            <AlertDescription>
              No analysis data available. Please ensure you've selected a valid
              analysis.
            </AlertDescription>
          </Alert>
        </div>
  ...
  return (
    <main className="flex-1 flex flex-col items-center justify-center gap-6 px-4 md:px-6 py-12">
      <div className="w-full max-w-4xl rounded-lg overflow-hidden">
        <Image
          src={`${apiUrl}/static/analysis/img/${analysis.map}.jpg`}
